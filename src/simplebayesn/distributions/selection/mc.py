...
def get_kde_interpolant_grids(m_sel, z_sel, m_com, z_com,
                              nm = 1000, nz = 1000,
                              eps = 1e-8):
    m_min, m_max = np.min(m_com), np.max(m_com)
    z_min, z_max = np.min(z_com), np.max(z_com)

    kde_sel = gaussian_kde(np.vstack([m_sel, z_sel]))
    kde_com = gaussian_kde(np.vstack([m_com, z_com]))

    def sel_prob_unnnorm(m, z, eps=eps):
        mz = np.vstack([m, z])
        return kde_sel(mz) / (kde_com(mz) + eps)
    
    integral = dblquad(sel_prob_unnnorm, z_min, z_max, m_min, m_max)[0]

    def sel_prob(m, z, eps=eps):
        return sel_prob_unnnorm(m, z, eps) / integral
    
    m_vec = np.linspace(m_min, m_max, nm)
    z_vec = np.linspace(z_min, z_max, nz)

    m_grid, z_grid = np.meshgrid(m_vec, z_vec, indexing='ij')

    positions = np.vstack([m_grid.ravel(), z_grid.ravel()])
    sel_prob_grid = sel_prob(positions[0], positions[1]).T.reshape(m_grid.shape)
    
    m_vec = jnp.asarray(m_vec)
    z_vec = jnp.asarray(z_vec)
    sel_prob_grid = jnp.asarray(sel_prob_grid)

    return m_vec, z_vec, sel_prob_grid

@jax.jit
def interpolate_selection_2d(m, z, m_vec, z_vec, sel_prob_grid):
    nm = len(m_vec)
    nz = len(z_vec)
    ...
    outside = ((m < m_min) | (m > m_max) | (z < z_min) | (z > z_max))
    
    m_idx = (m - m_min) / (m_max - m_min) * (nm - 1)
    ...
    val_00 = sel_prob_grid[m_i0, z_i0]
    val_01 = sel_prob_grid[m_i0, z_i1]
    val_10 = sel_prob_grid[m_i1, z_i0]
    val_11 = sel_prob_grid[m_i1, z_i1]
    
    val = (val_00 * (1 - m_frac) * (1 - z_frac) +
           val_01 * (1 - m_frac) * z_frac +
           val_10 * m_frac * (1 - z_frac) + 
           val_11 * m_frac * z_frac)
...
def log_selection_probability_mc_jax(tau, RB,
                                     ...
                                     m_grid = None, z_grid = None, sel_prob_grid = None,
                                     seed=0):
    ...
    if not use_kde_selection:
        ...
        p = interpolate_selection_2d(m_app_obs, observed_data_z[:, None],
                                     m_grid, z_grid, sel_prob_grid).mean(axis=1)
