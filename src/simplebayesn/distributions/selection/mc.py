...
def get_kde_interpolant_grids(c_sel, m_sel, c_com, m_com,
    ...
    kde_sel_c = gaussian_kde(c_sel)
    kde_sel_m = gaussian_kde(m_sel)
    kde_com_c = gaussian_kde(c_com)
    kde_com_m = gaussian_kde(m_com)

    def sel_prob_unnorm_c(c, eps=eps):
        return kde_sel_c(c) / (kde_com_c(c) + eps)
    
    def sel_prob_unnorm_m(m, eps=eps):
        return kde_sel_m(m) / (kde_com_m(m) + eps)
    
    integral_c = quad(sel_prob_unnorm_c, c_min, c_max)[0]
    integral_m = quad(sel_prob_unnorm_m, m_min, m_max)[0]

    def sel_prob_c(c, eps=eps):
        return sel_prob_unnorm_c(c, eps) / integral_c
    
    def sel_prob_m(m, eps=eps):
        return sel_prob_unnorm_m(m, eps) / integral_m
    
    c_vec = np.linspace(c_min, c_max, nc)
    m_vec = np.linspace(m_min, m_max, nm)

    sel_prob_grid_c = sel_prob_c(c_vec)
    sel_prob_grid_m = sel_prob_m(m_vec)
    
    c_vec = jnp.asarray(c_vec)
    m_vec = jnp.asarray(m_vec)
    sel_prob_grid_c = jnp.asarray(sel_prob_grid_c)
    sel_prob_grid_m = jnp.asarray(sel_prob_grid_m)

    return c_vec, m_vec, sel_prob_grid_c, sel_prob_grid_m

@jax.jit
def interpolate_selection_2d_ind(c, m, c_vec, m_vec, sel_prob_grid_c, sel_prob_grid_m):
    nc = len(c_vec)
    nm = len(m_vec)
    ...
    val_c_0 = sel_prob_grid_c[c_i0]
    val_c_1 = sel_prob_grid_c[c_i1]

    val_m_0 = sel_prob_grid_m[m_i0]
    val_m_1 = sel_prob_grid_m[m_i1]

    val_c = (val_c_0 * (1 - c_frac) +
             val_c_1 * c_frac)

    val_m = (val_m_0 * (1 - m_frac) +
             val_m_1 * m_frac)
    
    return jnp.where(outside, 0.0, val_c * val_m)
...
def log_selection_probability_mc_jax(tau, RB,
                                     ...
                                     c_grid = None, m_grid = None, sel_prob_grid_c = None, sel_prob_grid_m = None,
                                     seed=0):
    ...
    if not use_kde_selection:
        ...
        p = interpolate_selection_2d_ind(c_app_obs, m_app_obs,
                                     c_grid, m_grid, sel_prob_grid_c, sel_prob_grid_m).mean(axis=1)
